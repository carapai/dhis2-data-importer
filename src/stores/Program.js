    ...
    @action create = async () => {
        ...
        try {
            if (allInstances.length > 0) {
                const instancesResults = await this.insertTrackedEntityInstance({trackedEntityInstances: allInstances});
        ...
        try {
            if (newEnrollments.length > 0) {
                const enrollmentsResults = await this.insertEnrollment({enrollments: newEnrollments});
        ...
        try {
            if (allEvents.length > 0) {
                const eventsResults = await this.insertEvent({events: allEvents});
                    ...
                    if (orgUnits.length > 1) {
                        ...
                        if (orgUnit) {
                            if (enrollmentDates.length > 0 && this.isTracker && this.createNewEnrollments) {
                                ...
                                if (this.trackedEntityType && this.trackedEntityType.id) {
                                    tei = {...tei, trackedEntityType: this.trackedEntityType.id}
                                } else if (this.trackedEntity && this.trackedEntity.id) {
                                    tei = {...tei, trackedEntity: this.trackedEntity.id}
                                }
